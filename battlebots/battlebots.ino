// This code completes the battlebots challenge of the Robolympics. The robot will stay balanced even under pertrusion.
...
float thetaAccum = 0;
float angleoffset = 0.035; // offset from vertical for robot to be in equilibrium
...
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum, float delta_t) {
    float Kp = -6; // proportional control for angle
    float Ki = -36; // integral control for angle
    float Kd = 1; // proportional control for distance
    float Ldistance = totalDistanceLeft;
    float Rdistance = totalDistanceRight;
    float dist = Kd*(totalDistanceLeft + totalDistanceRight)/2;
    thetaAccum += -angleRad*delta_t;
    float vdL = Kp*(-angleRad - dist) + Ki*(thetaAccum);
    float vdR = Kp*(-angleRad - dist) + Ki*(thetaAccum);
    ...
    rightMotorPWM = Jp*vR_error + Ji*errorRightAccum;
}
...
void newBalanceUpdate()
{
  ...
  ax = alpha_ax*imu.a.x + (1-alpha_ax)*ax; // low pass filter to avoid motor shutting off for unexpected negative ax value
  if (ax < 0)
  {
    lyingDown();
...
/* Uncomment this and comment the above if doing wireless*/
  ...
  angle_rad = ((float)angle)/1000/180*3.14159 + angleoffset;
