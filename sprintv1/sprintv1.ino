// This code completes the sprint challenge of the robolympics.
...
float errorLeftAccum = 0;
float errorRightAccum = 0;
float thetaAccum = 0;
int totaltimeoffset = 180*1000; // 3 mins
float angleoffset =  0.035; //radians
...
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum, float delta_t, float cur_time) {
   ...
   */
    float thetad = - angleRad + angleoffset;
    float Kp = -6;
    float Ki = -36;
    float K = -1.0;
    float Ldistance = totalDistanceLeft;
    float Rdistance = totalDistanceRight;
    float dist = (totalDistanceLeft + totalDistanceRight)/2.0;
    float disttheta = K*(0.02*cur_time/1000.0 - dist);
    thetaAccum += (thetad)*delta_t;
    float vdL = Kp*(thetad - disttheta) + Ki*(thetaAccum);
    float vdR = Kp*(thetad - disttheta) + Ki*(thetaAccum);
    float vL_error = (vdL - vL);
    float vR_error = (vdR - vR);
    errorLeftAccum += vL_error*delta_t;
    errorRightAccum += vR_error*delta_t;
    float Jp = 500;
    float Ji = 5000;
    leftMotorPWM = Jp*vL_error + Ji*errorLeftAccum;
    rightMotorPWM = 0;
}
...
void newBalanceUpdate()
{
  ...
  Serial1.println(imu.a.x);

  ax = alpha_ax*imu.a.x + (1-alpha_ax)*ax;
  if (ax < 0)
  {
    lyingDown();
...
/* Uncomment this and comment the above if doing wireless
  ...
  if(cur_time - prev_time > UPDATE_TIME_MS && start_flag)
  {
    ...
    updatePWMs(totalDistanceLeft, totalDistanceRight, vL, vR, angle_rad, angle_rad_accum,delta_t, cur_time);
    ...
    //CheckTime(prev_time, totalDistanceLeft, totalDistanceRight);
  }
...
void CheckTime(float prev_time, float totalDistanceLeft, float totalDistanceRight){
  if (prev_time - lastchecktime > 10.0*1000){
